...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(Host_Changelevel2_f,
			...
			"Sven-8832",
			"81 ec c4 00 00 00 a1 ?? ?? ?? ?? 33 c4 89 84 24 c0 00 00 00 c7 05 ?? ?? ?? ?? 04 00 00 00 e8 ?? ?? ?? ?? 83 f8 02"
		);
		...
		PATTERNS(PF_GetPhysicsKeyValue,
			...
			"Sven-8832",
			"ff 74 24 04 e8 ?? ?? ?? ?? 83 c4 04 83 f8 01 7c ?? 3b 05 ?? ?? ?? ?? 7f ?? 0f ae e8"
		);
		...
		PATTERNS(Host_Reload_f,
			...
			"Sven-8832",
			"81 ec 08 01 00 00 a1 ?? ?? ?? ?? 33 c4 89 84 24 04 01 00 00 83 3d ?? ?? ?? ?? 00"
		);
	...
	namespace server
	{
		...
		PATTERNS(CNihilanth__DyingThink,
			"Sven-8832",
			"83 EC 78 A1 ?? ?? ?? ?? 33 C4 89 44 24 ?? A1 ?? ?? ?? ?? 56"
		);
	}
